...
def alphabeta_move(board: HexBoard, depth: int, is_max: bool, show_AI=True):
    """
    ...
    for move in legal_moves:
        ...
        if sim_board.check_win(
            ...
            return best_move
            #break
        score = alphabeta(sim_board, depth=depth, alpha=-np.inf, beta=np.inf,
    ...
    list_of_moves = dict(
        filter(lambda x: x[1] == best_score,
               list_of_moves.items()))  # filter out all moves that have a score below the best
    ...
    chosen_move =random.choice(list(list_of_moves.keys()))
    if show_AI: print(f"BEST MOVE: {chosen_move} with BEST SCORE: {best_score}")
    return chosen_move
...
def alphabeta_moveR(board: HexBoard, depth: int, is_max: bool, show_AI=False):
    ...
    for move in legal_moves:
        ...
        if sim_board.check_win(
            ...
            best_score = np.inf
            break
        score = alphabetaRandom(sim_board, depth=depth, alpha=-np.inf, beta=np.inf,
                                is_max=is_max)  # For some reason performs better if you use is_max=False
        if show_AI: print(f"CURRENT SCORE: {score} for MOVE: {move}")
        if score > best_score:
            best_score = score
